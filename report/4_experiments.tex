...
Therefore, we only included the SEM as error-bar on the figures. These error bars can also be seen as the confidence level of click models with respect to the evaluation measurement.

In \ref{fig:ll}, every click model has a high confidence level for the likelihood of each bin results. Therefore, we can conclude that all click models are statistically significant from one to the other in term of likelihood.

In \ref{fig:perp_qf} bin 1, all the click models show wider error bar compare with the other bin.
